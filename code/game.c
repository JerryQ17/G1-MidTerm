...
void game(void){
  ...
  game_init();
  current_state.round++;
  if (record) fprintf(rec_file, "Round%d %s %s", current_state.round, current_state.player_type == Player ? "Player" : "AI", current_state.cur_color);
...
void game_init(void){  //游戏部分的初始化
  ...
  current_state.player = INVALID;
  while (current_state.player > current_state.total_number) current_state.player = roll();
  //current_state.player_type;
  //初始化棋子
  chess_init();
  //渲染gameUI和棋子
  game_render();
...
int set_player(void){  //确定游戏人数
  ...
  while (SDL_WaitEvent(&SetPlayerEvent) && flag < 2) {
    main_render();
    ...
    switch (SetPlayerEvent.type) {
      ...
      case SDL_KEYDOWN: //按下键盘
        ...
        SDL_Delay(2000);  //给玩家显示2s输入的值
        break;
      case SDL_MOUSEBUTTONDOWN:
        if (record) fprintf(rec_file, "SetPlayer:Mouse button down (%d, %d)\n", SetPlayerEvent.button.x, SetPlayerEvent.button.y);
        break;
        //本函数仍在mainUI里，能看见开始和帮助按钮，只保留帮助功能
      case SDL_MOUSEBUTTONUP:
        if (record) fprintf(rec_file, "SetPlayer:Mouse button up (%d, %d)\n", SetPlayerEvent.button.x, SetPlayerEvent.button.y);
        if (SetPlayerEvent.button.x > 555 && SetPlayerEvent.button.x < 722 && SetPlayerEvent.button.y > 463 && SetPlayerEvent.button.y < 547){ //帮助
